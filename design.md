...
6. PromptTemplate visibility consistency: a PRIVATE template cannot be added to `WorkspacePromptTemplate`. Enforced by a pre-insert check in `PromptMenu.java` (`shareTemplate`).
...
| **MessageFeedback**         | feedback_id                 | message_id --> Message (CASCADE, UNIQUE)                                                                                    | rating (CHECK IN ('Thumbs Up','Thumbs Down')), comment_text (nullable), submitted_at                                                                                                      | App check (ConversationMenu.java): message must have sender_role = 'AI' |
...
| **WorkspacePromptTemplate** | (workspace_id, template_id) | workspace_id --> Workspace (CASCADE), template_id --> PromptTemplate (CASCADE)                                              | shared_at                                                                                                                                                                                 | Composite PK; app check (PromptMenu.java): template must have visibility = 'SHARED' |
